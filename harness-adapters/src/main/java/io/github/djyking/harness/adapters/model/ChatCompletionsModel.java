...
/**
 * Non-streaming chat-completions protocol with explicit provider-specific token limits. Transport
 * uncertainty remains UNKNOWN; provider response bodies are never included in errors.
 */
public final class ChatCompletionsModel implements ModelGateway {
  private final ChatCompletionsConfig config;
  private final HttpClient client;

  public ChatCompletionsModel(ChatCompletionsConfig config) {
  ...
  private ObjectNode encode(ModelRequest request) {
    ...
    body.put(config.tokenLimitParameter(), request.profile().maxOutputTokens());
    if (config.disableThinking()) body.set("thinking", Json.object().put("type", "disabled"));
